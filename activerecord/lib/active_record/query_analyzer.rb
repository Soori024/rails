...
module ActiveRecord
  ...
  # duplicate, potential N+1, and slow query patterns. Intended for development
  ...
  module QueryAnalyzer
    ...
    #
    # IsolatedExecutionState keeps collectors per-thread, or per-fiber depending
    # on ActiveSupport.isolation_level, so concurrent requests never share or
    # overwrite one another's metrics.
    ...
    class << self
      ...
      # How many times a parameterized SELECT must repeat before it is flagged
      # as a potential N+1. Defaults to 5.
      ...
      # which bounds memory use. Defaults to 1000.
      attr_accessor :max_tracked_queries

      # A callable invoked with the Report at the end of each unit of execution.
      # Defaults to +nil+, in which case the report goes to the Active Record
      # logger.
      ...
      #
      # The subscriber is attached here rather than only at boot so that a
      # caller driving the analyzer directly -- including the executor hooks --
      # actually collects queries instead of silently reporting none.
      def start
        install_subscriber
      ...
      def analyze
        ...
        begin
          ...
          if previous
            ...
            # Delete rather than assign nil, so a thread that called analyze
            # once isn't left holding a permanent entry.
            ActiveSupport::IsolatedExecutionState.delete(STATE_KEY)
          end
        end
      end
      ...
      # Subscribes to +sql.active_record+. Safe to call repeatedly.
      def install_subscriber
        # Lock-free fast path: once subscribed, @subscriber only returns to nil
        # through uninstall_subscriber, so the common case avoids the mutex.
        return if @subscriber

        # start calls this on every invocation, so two threads can reach an
    ...
    #
    # A module responding to +call+ rather than an ActiveSupport::LogSubscriber,
    # so it receives monotonic timings and skips the event-object allocation the
    # log subscriber path performs.
    ...
    # request or job without it needing to know anything about Action Pack or
    # Active Job.
