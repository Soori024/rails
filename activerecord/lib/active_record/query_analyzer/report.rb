...
module ActiveRecord
  module QueryAnalyzer
    ...
    #
    # An immutable snapshot of what a Collector observed during one unit of
    ...
    # surface of the feature -- deliberately free of the collector's internals
    # so custom reporters have something stable to build on.
    class Report # :nodoc:
      ...
      # Total redundant executions across all duplicated shapes.
      ...
      # A Hash representation, for structured logging or for shipping the report
      ...
      # A multi-line, human-readable summary, or an empty string when no queries
      # were recorded. Use empty? to decide whether to log at all.
      def to_s
        return "" if empty?

        lines = [header]

        # Findings come most severe first: an N+1 is usually the cause of the
        # duplicates reported below it, so it is the thing worth reading first.
      ...
      private
        ...
        def slow_query_section
          lines = ["  Slow queries (over #{format_ms(QueryAnalyzer.slow_query_threshold)}):"]
