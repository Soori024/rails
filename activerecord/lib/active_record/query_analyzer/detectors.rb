...
module ActiveRecord
  module QueryAnalyzer
    ...
    # so a detector can be exercised against a handful of hand-built stats with
    # no database involved.
    #
    # Keeping the heuristics here rather than on Collector means adding an
    ...
    module Detectors # :nodoc:
      # Shapes executed more than once. Every execution beyond the first is
      # redundant work, whatever the cause.
      ...
      # health check, or a repeated `SELECT * FROM settings LIMIT 1` -- and is
      # the main thing keeping the false positive rate down.
      module NPlusOne # :nodoc:
        ...
        def call(stats)
          ...
          stats.select { |stat|
            ...
            # A truncated shape may have lost its only placeholder to the
            # length cap. Repeating an identical constant SELECT that long is
            # not a realistic pattern, so treat it as a candidate rather than
            # drop a genuine N+1 on a technicality.
      ...
      #
      # The worst execution is compared rather than the average, so a shape that
      # is usually fast but occasionally slow still surfaces. Cached queries are
      # excluded: a query cache hit does no database work, so reporting one as
      # slow would point at the wrong thing.
